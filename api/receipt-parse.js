...
- "tax" is ONLY government tax (VAT, MwSt., Sales Tax, GST, IVA, etc.) explicitly labeled as such on the receipt. If you do not see a tax line, set "tax" to null.
- "service" is ONLY a service charge or tip explicitly labeled as such. If you do not see one, set "service" to null.
- DO NOT put discounts, rebates, "Werberabatt", "Rabatt", "Discount", "-XX%", store credits, or membership savings into tax/service. These are price reductions, not extras. If items already reflect the discounted price, ignore the discount line entirely. If items are pre-discount, REDUCE individual item prices so the line totals match what was actually charged.
- Sanity-check: items × qty (after any discount adjustments) + tax + service should equal total. If it doesn't, prefer adjusting item prices over inventing tax/service.
- Skip non-item lines (subtotal, total, payment method, change, "Bon Nr", "Kasse").
