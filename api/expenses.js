...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    const category = body.category ? String(body.category).slice(0, 20) : null;
    const receiptId = body.receiptId ? String(body.receiptId).slice(0, 32) : null;
    const vendor = body.vendor ? String(body.vendor).slice(0, 60) : null;
