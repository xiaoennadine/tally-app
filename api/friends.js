...
module.exports = async (req, res) => {
  ...
  if (req.method === 'PATCH') {
    ...
    if (typeof body.avatarEmoji === 'string') friend.avatarEmoji = body.avatarEmoji.slice(0, 4);
    if (typeof body.name === 'string' && body.name.trim()) friend.name = body.name.trim().slice(0, 40);
    await saveWallet(userId, wallet);
    return res.status(200).json({ member: friend });
  }
