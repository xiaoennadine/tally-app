...
function SettleCard({ tx, userId, wallet, onSettled, direction, toHome, homeCcy }) {
  ...
  // Nudge: open Telegram/WhatsApp share with a friendly pre-filled reminder.
  const nudge = () => {
    const name = (other?.name || 'there').split(' ')[0];
    const amt = fmtMoney(tx.amount, tx.ccy);
    const msg = `Hey ${name}! Friendly reminder to settle our ${amt} on Tally 😊`;
    setNudged(true);
    try { navigator.clipboard?.writeText(msg); } catch (e) {}
    // Prefer Telegram's native share inside the Mini App; fall back to WhatsApp.
    const tg = window.Telegram?.WebApp;
    if (tg?.openTelegramLink) {
      tg.openTelegramLink(`https://t.me/share/url?url=${encodeURIComponent(' ')}&text=${encodeURIComponent(msg)}`);
    } else {
