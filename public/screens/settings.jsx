...
function SettingsScreen({ data, onBack, onChanged }) {
  ...
  const doReset = async () => {
    ...
    setResetting(true);
    try {
      await TallyAPI.settings({ reset: true });
      await onChanged();
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: T_st.bg }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', paddingBottom: 100 }} className="no-scroll">
        ...
        <div style={{ padding: '16px 14px 0' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: T_st.negative, letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 8 }}>
            Danger zone
          </div>
          <Card>
            {!confirmReset ? (
              <div onClick={() => setConfirmReset(true)} style={{
                padding: '14px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer',
              }}>
                <div style={{
                  width: 36, height: 36, borderRadius: 9, background: '#fee2e2', color: T_st.negative,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 17,
                }}>🗑</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: T_st.negative }}>Reset all data</div>
                  <div style={{ fontSize: 11.5, color: T_st.secondary, marginTop: 1 }}>Clears expenses, friends, groups & subscriptions</div>
                </div>
              </div>
            ) : (
              <div style={{ padding: 14 }}>
                <div style={{ fontSize: 13, color: T_st.text, fontWeight: 600, marginBottom: 4 }}>Reset everything?</div>
                <div style={{ fontSize: 12, color: T_st.secondary, lineHeight: 1.5, marginBottom: 12 }}>
                  This permanently deletes all expenses, friends, groups, and subscriptions. Your name, avatar, and home currency are kept. This can't be undone.
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Button onClick={() => setConfirmReset(false)} variant="ghost" style={{ flex: 1, padding: '10px' }}>Cancel</Button>
                  <Button onClick={doReset} variant="danger" disabled={resetting} style={{ flex: 1, padding: '10px' }}>
