...
function HomeScreen({ data, onAdd, onScanReceipt, onSettle, onFriends, onSubs, onGroups, onSettings, onSelectGroup, selectedGroupId, onRemoveExpense, onOpenExpense }) {
      ...
      <div style={{ flex: 1, overflow: 'auto', paddingBottom: 160 }} className="no-scroll">
        ...
        <div style={{ padding: '0 14px' }}>
          ...
          {visibleExpenses.length > 0 && (
            <Card>
              {(() => {
                // Group expenses by receiptId so multi-line receipts show as ONE row.
                // Expenses without a receiptId stay as individual rows.
                const grouped = [];
                const seenReceipt = new Set();
                for (const e of visibleExpenses) {
                  if (e.receiptId) {
                    if (seenReceipt.has(e.receiptId)) continue;
                    seenReceipt.add(e.receiptId);
                    const sibs = visibleExpenses.filter(x => x.receiptId === e.receiptId);
                    const totalAmt = sibs.reduce((a, x) => a + x.amount, 0);
                    // Aggregate splitWith / payer: should all match for receipt-siblings
                    const allSplit = new Set();
                    for (const s of sibs) for (const id of s.splitWith) allSplit.add(id);
                    const myShareTotal = sibs.reduce((a, x) => a + (x.splitWith.includes(userId) ? x.amount / x.splitWith.length : 0), 0);
                    const myPaidTotal = sibs.filter(x => x.payer === userId).reduce((a, x) => a + x.amount, 0);
                    grouped.push({
                      id: e.id,
                      receiptId: e.receiptId,
                      payer: e.payer,
                      vendor: e.vendor || e.desc.split(' — ')[0] || 'Receipt',
                      desc: `${e.vendor || 'Receipt'} (${sibs.length} ${sibs.length === 1 ? 'item' : 'splits'})`,
                      amount: totalAmt,
                      ccy: e.ccy,
                      date: e.date,
                      groupId: e.groupId,
                      category: e.category,
                      splitWithSize: allSplit.size,
                      yourImpact: myPaidTotal - myShareTotal,
                      isReceipt: true,
                    });
                  } else {
                    const share = e.splitWith.includes(userId) ? e.amount / e.splitWith.length : 0;
                    const yourImpact = (e.payer === userId) ? (e.amount - share) : -share;
                    grouped.push({
                      ...e,
                      splitWithSize: e.splitWith.length,
                      yourImpact,
                      isReceipt: false,
                    });
                  }
                ...
                return grouped.map((e, i) => {
                  ...
                  const impactInHome = isForeign && fxReady
                    ? window.convertMoney(e.yourImpact, e.ccy, homeCcy, rates)
                    : null;
                  return (
                    <Row key={e.id} last={i === grouped.length - 1} onClick={() => onOpenExpense && onOpenExpense(e.id)}>
                      <div style={{
                        ...
                        {e.isReceipt && (
                          <div style={{
                            ...
                            fontSize: 9, boxShadow: '0 0 0 1.5px #fff',
                          }}>📄</div>
                        )}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11.5, color: T_home.secondary }}>
                          {isYou ? 'You' : payer?.name || '?'} paid · {fmtMoney(e.amount, e.ccy)} · ÷{e.splitWithSize}
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: e.yourImpact >= 0 ? T_home.positive : T_home.negative, fontVariantNumeric: 'tabular-nums' }}>
                          {e.yourImpact >= 0 ? '+' : '−'}{fmtMoney(Math.abs(e.yourImpact), e.ccy)}
                        </div>
                        {impactInHome !== null && (
