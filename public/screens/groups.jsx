          ...
          {groups.length > 0 && (
            <Card>
              {groups.map((g, i) => {
                ...
                return (
                  <Row key={g.id} last={i === groups.length - 1} onClick={() => onOpenGroup(g.id)}>
          ...
          {allMembers.map(m => {
            ...
            return (
              <div key={m.id} onClick={() => toggle(m.id)} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}><Avatar name={m.name} emoji={m.avatarEmoji} size={32} /></div>
