...
// Demo receipts so the scan flow is testable offline and can be offered as a
// graceful fallback if the OCR endpoint is unreachable. Each becomes the same
// shape the OCR returns: { vendor, currency, items[], tax, service, total }.
const PRESET_RECEIPTS = [
  {
    vendor: '🍣 Tokyo Sushi Tavern', currency: 'JPY', category: 'food',
    items: [
      { name: 'Wagyu Beef Steak', qty: 1, price: 4200 },
      { name: 'Premium Uni Sushi', qty: 1, price: 3200 },
      { name: 'Matcha Fondant Cake', qty: 1, price: 1800 },
      { name: 'Double Chocolate Gelato', qty: 1, price: 1200 },
      { name: 'Edamame (shared side)', qty: 1, price: 600 },
      { name: 'Gyoza (shared side)', qty: 1, price: 1000 },
      { name: 'Sapporo Draft Pitcher', qty: 1, price: 2200 },
    ],
    tax: 1420, service: 800, total: 16420,
  },
  {
    vendor: '🥞 Brooklyn Diner', currency: 'USD', category: 'food',
    items: [
      { name: 'Pancakes & Maple Syrup', qty: 1, price: 14.5 },
      { name: 'Eggs Benedict', qty: 1, price: 16.5 },
      { name: 'Avocado Toast Deluxe', qty: 1, price: 15.5 },
      { name: 'Loaded Waffle (shared)', qty: 1, price: 12 },
      { name: 'Chemex Coffee Bottle', qty: 1, price: 9 },
    ],
    tax: 4.5, service: 6, total: 78,
  },
  {
    vendor: '🚄 Shinkansen Tokyo→Kyoto', currency: 'JPY', category: 'travel',
    items: [
      { name: 'Seat ticket · person 1', qty: 1, price: 14100 },
      { name: 'Seat ticket · person 2', qty: 1, price: 14100 },
      { name: 'Seat ticket · person 3', qty: 1, price: 14150 },
    ],
    tax: 0, service: 0, total: 42350,
  },
];
...
function ReceiptScreen({ data, onClose, onSaved, onRefresh }) {
  ...
  const onPickFile = async (e) => {
    ...
    // Photo-OCR needs Telegram's signed initData to authorise the (paid) API call.
    // Opened in a plain browser there's none, so don't even try — guide to manual/sample.
    const initData = window.Telegram?.WebApp?.initData || '';
    if (!initData) {
      setError('Photo scanning works inside the Telegram app. Here you can enter items manually or start from a sample.');
      setStep('error');
      return;
    }
  ...
  // Load a blank receipt so the user can build items by hand — works with no API.
  const loadBlank = () => {
    const items = [{ id: 'i0', name: '', qty: 1, price: 0 }];
    setReceipt({
      vendor: '', currency: wallet.defaultCcy || 'USD',
      tax: 0, service: 0, total: 0, items,
    });
    setAssigned({ i0: new Set([userId]) });
    setError(null);
    setStep('review');
  };

  // Load a demo receipt into the review flow (used by presets + fallback).
  const loadPreset = (preset) => {
    const items = preset.items.map((it, i) => ({
      id: `i${i}`, name: it.name, qty: it.qty || 1, price: it.price,
    }));
    setReceipt({
      vendor: preset.vendor, currency: preset.currency,
      tax: preset.tax || 0, service: preset.service || 0, total: preset.total,
      items,
    });
    const a = {};
    for (const it of items) a[it.id] = new Set([userId]);
  ...
  const toggleAssign = (itemId, memberId) => {    setAssigned(prev => {
  ...
  const addFriendInline = async () => {
    ...
    try {
      ...
      if (added.length) {
        ...
        setSplitSet(s);
        // Default-assign the new friend to every item, same as togglePresence does
        const a = { ...assigned };
        for (const k of Object.keys(a)) {
          const set = new Set(a[k]); set.add(f.id); a[k] = set;
        }
        setAssigned(a);
      }
      setNewName('');
  ...
  const togglePresence = (memberId) => {
    ...
    if (s.has(memberId)) {
      ...
      s.add(memberId);
      // Adding someone to "who was here" defaults them onto every item too.
      // Users can then untap them from specific items they didn't eat.
      // This avoids the trap of: add 3 friends → still see €0 for each.
      const a = { ...assigned };
      for (const k of Object.keys(a)) {
        const set = new Set(a[k]); set.add(memberId); a[k] = set;
      }
      setAssigned(a);
    }
    setSplitSet(s);
  ...
  const removeItem = (itemId) => {
    setReceipt(r => ({ ...r, items: r.items.filter(it => it.id !== itemId) }));
    setAssigned(prev => {
      const next = { ...prev };
      delete next[itemId];
      return next;
  ...
  const addItem = () => {
    ...
    setReceipt(r => ({
      ...r,
      items: [...r.items, { id: newId, name: 'New item', qty: 1, price: 0 }],
    }));
    setAssigned(prev => ({ ...prev, [newId]: new Set([userId]) }));
  };

  // Compute per-person totals (apportion tax/service proportionally to item share).
  //
  // We only apply tax/service if doing so brings the items closer to OCR's stated total.
  // OCR sometimes hallucinates tax from discount lines (e.g. Werberabatt) which would
  // wrongly inflate everyone's share. If extras > 0 but items already match the receipt
  // total, OR if applying extras would push us PAST the receipt total, we ignore them.
  const perPerson = React.useMemo(() => {
    ...
    // Decide whether to apply tax/service.
    // Trust extras only if items + extras ≈ stated total (i.e. the math actually closes).
    const claimedExtras = (receipt.tax || 0) + (receipt.service || 0);
    const ocr = receipt.total || 0;
    let extras = 0;
    if (claimedExtras > 0 && ocr > 0) {
      const withExtras = Math.abs(subSum + claimedExtras - ocr);
      const withoutExtras = Math.abs(subSum - ocr);
      // Use extras only if they bring us closer to the OCR total
      if (withExtras < withoutExtras) extras = claimedExtras;
    } else if (claimedExtras > 0 && !ocr) {
      // No OCR total to validate against; trust extras
      extras = claimedExtras;
    }
  ...
  const save = async () => {
    ...
    try {
      ...
      // Apportion tax/service only if doing so brings items closer to the receipt total.
      // (Prevents OCR's hallucinated tax from discount lines like "Werberabatt" inflating shares.)
        ...
        <div style={{ padding: '20px 16px 30px', textAlign: 'center' }}>
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 12, justifyContent: 'center', alignItems: 'center' }}>
            ...
            <label htmlFor="upload-receipt" style={{
              fontSize: 13, color: T_r.primary, fontWeight: 600, cursor: 'pointer',
            }}>upload from photos</label>
            <span style={{ color: T_r.divider }}>·</span>
            <span onClick={loadBlank} style={{
              fontSize: 13, color: T_r.primary, fontWeight: 600, cursor: 'pointer',
            }}>enter manually</span>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '14px 4px 12px' }}>
            <div style={{ flex: 1, height: 1, background: T_r.divider }} />
            <div style={{ fontSize: 11, color: T_r.muted, fontWeight: 600 }}>or try a sample</div>
            <div style={{ flex: 1, height: 1, background: T_r.divider }} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, textAlign: 'left' }}>
            {PRESET_RECEIPTS.map((p, i) => (
              <div key={i} onClick={() => loadPreset(p)} style={{
                display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px',
                background: '#f4f5f7', borderRadius: 12, cursor: 'pointer',
              }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13.5, fontWeight: 600, color: T_r.text }}>{p.vendor}</div>
                  <div style={{ fontSize: 11, color: T_r.secondary, marginTop: 1 }}>{p.items.length} items · {fmtMoney(p.total, p.currency)}</div>
                </div>
                <svg width="16" height="16" viewBox="0 0 20 20" style={{ color: T_r.muted }}><path d="M7 4l6 6-6 6" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </div>
            ))}
          </div>
        </div>
  ...
  if (step === 'error') {
    return (
      <Sheet open={true} onClose={onClose} title="Couldn't scan that">
        <div style={{ padding: '20px 18px 26px' }}>
          <div style={{ textAlign: 'center', marginBottom: 18 }}>
            <div style={{ fontSize: 44, marginBottom: 8 }}>😕</div>
            <div style={{ fontSize: 14, color: T_r.text, fontWeight: 600 }}>{error || 'Could not read the receipt.'}</div>
            <div style={{ fontSize: 12.5, color: T_r.secondary, marginTop: 6, lineHeight: 1.5 }}>
              Retake the photo, or start from a sample receipt and edit the items.
            </div>
          </div>
          <Button onClick={() => { setStep('capture'); setError(null); }} style={{ width: '100%' }}>📷 Try again</Button>
          <div onClick={loadBlank} style={{
            marginTop: 10, textAlign: 'center', fontSize: 13, color: T_r.primary, fontWeight: 600, cursor: 'pointer', padding: 6,
          }}>✏️ Enter items manually instead</div>

          <div style={{ fontSize: 11, color: T_r.secondary, letterSpacing: 0.5, textTransform: 'uppercase', fontWeight: 700, margin: '18px 0 8px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {PRESET_RECEIPTS.map((p, i) => (
              <div key={i} onClick={() => loadPreset(p)} style={{
                ...
                background: '#fff', border: `1px solid ${T_r.divider}`, borderRadius: 12, cursor: 'pointer',
              }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13.5, fontWeight: 600, color: T_r.text }}>{p.vendor}</div>
                  <div style={{ fontSize: 11, color: T_r.secondary, marginTop: 1 }}>{p.items.length} items · {fmtMoney(p.total, p.currency)}</div>
    ...
    <Sheet open={true} onClose={onClose} title="Review split">
      <div style={{ padding: '4px 16px 24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          <label style={{ fontSize: 11, color: T_r.secondary, letterSpacing: 0.5, textTransform: 'uppercase' }}>Items — tap to edit, assign who had each</label>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 6, marginBottom: 10 }}>
          {receipt.items.map(it => {
            ...
            const ccySym = window.SYM_OF?.[receipt.currency || 'USD'] || (receipt.currency === 'JPY' ? '¥' : '$');
            return (
              <div key={it.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <input
                    ...
                    style={{
                      flex: 1, fontSize: 14, color: T_r.text, fontWeight: 500,
                      border: 'none', outline: 'none', background: 'transparent', padding: 0, minWidth: 0,
                    }}
                  />
                  <div onClick={() => removeItem(it.id)} title="Remove" style={{
                    width: 22, height: 22, borderRadius: '50%',
                    color: T_r.muted, fontSize: 13, cursor: 'pointer',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    flexShrink: 0,
                  }}>×</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                  <div style={{ fontSize: 11, color: T_r.muted }}>qty</div>
                  <input
                    type="number" inputMode="numeric" min="1"
                    value={it.qty}
                    onChange={(e) => updateItem(it.id, { qty: Math.max(1, parseInt(e.target.value) || 1) })}
                    style={{
                      width: 36, fontSize: 12, padding: '3px 6px',
                      border: `1px solid ${T_r.divider}`, borderRadius: 6, outline: 'none', textAlign: 'center',
                    }}
                  />
                  <div style={{ fontSize: 11, color: T_r.muted }}>×</div>
                  <div style={{ position: 'relative', flex: '0 0 auto' }}>
                    <span style={{ position: 'absolute', left: 6, top: 3, fontSize: 12, color: T_r.muted, pointerEvents: 'none' }}>{ccySym}</span>
                    <input
                      type="number" inputMode="decimal" step="0.01" min="0"
                      value={it.price}
                      onChange={(e) => updateItem(it.id, { price: parseFloat(e.target.value) || 0 })}
                      style={{
                        width: 70, fontSize: 12, padding: '3px 6px 3px 16px',
                        border: `1px solid ${T_r.divider}`, borderRadius: 6, outline: 'none', textAlign: 'right',
                        fontVariantNumeric: 'tabular-nums',
                      }}
                    />
                  </div>
                  <div style={{ flex: 1, textAlign: 'right', fontSize: 12, fontWeight: 600, color: T_r.text, fontVariantNumeric: 'tabular-nums' }}>
                    = {fmtMoney(total, receipt.currency || 'USD')}
                    {who.size > 1 && <span style={{ color: T_r.muted, fontWeight: 400 }}> · {fmtMoney(total / who.size, receipt.currency || 'USD')} ea</span>}
                  </div>
                </div>
                  ...
                  {[...splitSet].map(id => {
                    ...
                    return (
                      <div key={id} onClick={() => toggleAssign(it.id, id)} style={{
                        ...
                        <Avatar name={m.name} emoji={m.avatarEmoji} size={16} />
        ...
        <div onClick={addItem} style={{
          ...
          background: 'transparent', border: `1px dashed ${T_r.muted}`,
          color: T_r.secondary, fontSize: 13, fontWeight: 500,
          textAlign: 'center', cursor: 'pointer',
        }}>+ Add a missing item</div>
        ...
        <Card style={{ marginTop: 6, marginBottom: 14 }}>
          {Object.entries(perPerson).map(([id, amt], i, arr) => (
            <Row key={id} last={i === arr.length - 1}>
              <Avatar name={wallet.members[id]?.name || '?'} emoji={wallet.members[id]?.avatarEmoji} size={28} />
