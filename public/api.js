...
window.TallyAPI = {
  ...
  friends: {
    ...
    patch:  (id, body)       => api(`/api/friends?id=${encodeURIComponent(id)}`, { method: 'PATCH', body }),
...
// ─── Offline demo mode ────────────────────────────────────────────────
// When opened outside Telegram with no dev server, swap in the localStorage
// mock so the app is fully previewable. Production (real backend + Telegram
// initData, or ?devUser=) keeps the real endpoints.
const OFFLINE = !(tg && tg.initData) && !devUser();
if (OFFLINE && window.TallyMock) {
  const realFX = window.TallyAPI.fx;
  window.TallyAPI = window.TallyMock.buildAPI(tg);
  window.TallyMock.active = true;
}
...
// Categories — applied to expenses for the breakdown chart.
window.EXPENSE_CATEGORIES = [
  { id: 'food',    label: 'Food',     icon: '🍱', color: '#ff6b8b' },
...
window.getCategory = function(id) {
  return window.EXPENSE_CATEGORIES.find(c => c.id === id) || window.EXPENSE_CATEGORIES[5];
};
window.TG_TOKENS = {
